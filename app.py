...
def generate_speech(preset_name, description, text, temperature, max_tokens, seed):
...
with gr.Blocks(title="Maya1 - Open Source Emotional TTS", theme=gr.themes.Soft()) as demo:
    ...
    **The best open source voice AI model with emotions!**
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            preset_dropdown = gr.Dropdown(
                choices=["Custom"] + list(PRESET_CHARACTERS.keys()),
                label="Preset Characters",
                value="Custom",
                info="Select a preset or use Custom to design your own voice"
            )
            
            gr.Markdown("### Voice Design")
            
            gr.Markdown("""
            **Tip:** For best results with custom voices, include: age, accent, pitch, timbre, pacing, tone, intensity, domain, role, and delivery style.
            """)
            
            description_input = gr.Textbox(
                label="Voice Description",
                placeholder="E.g., Male voice in their 30s with american accent. Normal pitch, warm timbre, conversational pacing, friendly tone at medium intensity, podcast domain, host role, casual delivery",
                lines=4,
                value=""
            )
            ...
            with gr.Accordion("Advanced Settings", open=False):
                ...
                seed_input = gr.Number(
                    ...
                    value=-1,
                    precision=0,
                    info="Set to -1 for random, or use a specific number for reproducible results"
                )
        ...
        with gr.Column(scale=1):
            ...
            `<angry>` `<chuckle>` `<cry>` `<curious>` `<disappointed>` `<excited>` 
            `<exhale>` `<gasp>` `<giggle>` `<gulp>` `<laugh>` `<laugh_harder>` 
            `<mischievous>` `<sarcastic>` `<scream>` `<sigh>` `<sing>` `<snort>` `<whisper>`
            
            Use these tags in your text to add emotional expression to the speech.
            """)
    ...
    generate_btn.click(
        fn=generate_speech,
        inputs=[preset_dropdown, description_input, text_input, temperature_slider, max_tokens_slider, seed_input],
